...
void Update(void)
{
	...
	case STAGE_TITLE:					// タイトル
		SetPlayerAnime(0, PLAYER_ANIME_HAND, SKIN_ANIME_WEIGHT_PLAYER_NON);
		UpdateMeshcloud();		// 雲海
		UpdateShadow();			// 影
		UpdateModel();			// モデル
		UpdatePlayer();			// プレイヤー
	...
	case STAGE_GAME:					// ゲーム
		if (!stageSys->bEndFlag)
		{
			...
			else
			{
				...
				player->m_CSkinMesh.Update(GetPlayerMtx());
			}
		...
		else
		{
			...
			if (stageSys->nEndCount == 0)
			{
				...
				else
				{
					...
					SetPlayerAnime(0, PLAYER_ANIME_DEATH, SKIN_ANIME_WEIGHT_PLAYER_FAST);
				}
			}
